...
public class DroidChessController {
    ...
    private synchronized void makeComputerMove(int id, final String cmd, final Move ponder) {
        ...
        if (res.first) {
            updateComputeThreads();
            if (!RLTask.get().isEnabled()) { // no UI for computer ;)
                setSelection();
            }
        ...
        for (Move m : moves) {
            if ((m.from == move.from) && (m.to == move.to)) {
                if ((m.promoteTo != Piece.EMPTY) && (promoteTo == Piece.EMPTY)) {
                    ...
                    RLTask.get().logExtra("promote [" + m.from + ", " + m.to + "]");
                    return false;
                }
                if (m.promoteTo == promoteTo) {
                    ...
                    RLTask.get().logExtra("moved [" + m.from + ", " + m.to + "]");
                    return true;
                }
            }
        }
        RLTask.get().logExtra("invalid [" + move.from + ", " + move.to + "]");
    ...
    private void updateGUI() {
        ...
        if (s.state != GameState.ALIVE) {
            if (s.state == GameState.BLACK_MATE) {
                RLTask.get().logScore(
                    gameMode.playerWhite() ?
                        0 : // computer won
                        1);  // agent won
            }
            if (s.state == GameState.WHITE_MATE) {
                RLTask.get().logScore(
