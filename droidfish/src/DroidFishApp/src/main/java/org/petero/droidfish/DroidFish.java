...
public class DroidFish extends Activity
    ...
    GameMode gameMode;
    private Random nextPlayer = new Random();
    ...
    private void readPrefs(boolean restartIfLangChange) {
        ...
        if (RLTask.get().isEnabled()) {
            modeNr = readNewGameModeType(modeNr);
        }
        ...
        if (RLTask.get().isEnabled()) {
            int customStrength = RLTask.get().get("strength_percent", 100);
            if (customStrength >= 0 & customStrength <= 100) {
                settings.edit()
                    .putInt(
                        "strength",
                        customStrength * 10
                ...
                RLTask.logDebug(this, "Ignoring invalid custom strength '" + customStrength + "'!");
            }
        }
        ...
        if (RLTask.get().isEnabled()) {
            int theme = RLTask.get().get("theme", 2);
            if (theme >= 0 && theme <= 6) {
                ColorTheme.instance().setTheme(
                    settings,
                    theme
                    );
            } else {
                RLTask.logDebug(this, "Ignoring invalid custom theme '" + theme + "'!");
            }
        }
        ColorTheme.instance().readColors(settings);
        if (RLTask.get().isEnabled()) {
            String pieceSet = RLTask.get().get("viewPieceSet", "chesscases");
            if (PieceSet.instance().availPieceSets.contains(pieceSet)) {
                settings.edit()
                    .putString(
                        "viewPieceSet",
                        pieceSet
                ...
                RLTask.logDebug(this, "Ignoring invalid custom piece set '" + pieceSet + "'!");
            }
        }
    ...
    private void newGameMode(int gameModeType) {
        Editor editor = settings.edit();
        if (RLTask.get().isEnabled()) {
            gameModeType = readNewGameModeType(gameModeType);
        }
    ...
    private int readNewGameModeType(int gameModeType) {
        gameModeType = RLTask.get().get("gameMode", gameModeType);
        if (gameModeType == 0) {
            gameModeType = nextPlayer.nextBoolean() ? GameMode.PLAYER_WHITE : GameMode.PLAYER_BLACK;
        }
        return gameModeType;
    }
