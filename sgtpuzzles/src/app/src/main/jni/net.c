                               ...
                               const game_state *newstate)
{
#ifdef ANDROID
    char *gridExtra;
    gridExtra = malloc(newstate->width * newstate->height * 4 * sizeof(char));
    strcpy(gridExtra, "[");
    char *gridCompletedExtra;
    gridCompletedExtra = malloc(newstate->width * newstate->height * 4 * sizeof(char));
    strcpy(gridCompletedExtra, "[");
    unsigned char *active;
    int pos = 0;
    int w, h;
    int activeCount = 0;
    ...
    int middleW = (newstate->width - 1) / 2;
    int middleH = (newstate->height - 1) / 2;
    active = compute_active(newstate, middleW, middleH);
    for (h = 0; h < newstate->height; h++) {
        if (h > 0) {
            strcat(gridExtra, ", ");
            strcat(gridCompletedExtra, ", ");
        }
        strcat(gridExtra, "[");
        strcat(gridCompletedExtra, "[");
        for (w = 0; w < newstate->width; w++) {
            tile = newstate->tiles[pos];
            if (w > 0) {
                strcat(gridExtra, ", ");
                strcat(gridCompletedExtra, ", ");
            }
            sprintf(string, "%d", tile);
            strcat(gridExtra, string);
            if ((tile & 0xF) && !active[pos]) {
                strcat(gridCompletedExtra, "0");
            } else {
                activeCount++;
                strcat(gridCompletedExtra, "1");
            }
            activeTotal++;
            pos++;
        }
    ...
    strcat(gridExtra, "]");
    strcat(gridCompletedExtra, "]");
    android_game_state_changed_int("active", activeCount);
    android_game_state_changed_int("total", activeTotal);
    android_game_state_changed("grid", gridExtra);
    android_game_state_changed("gridCompleted", gridCompletedExtra);
    sfree(gridExtra);
    sfree(gridCompletedExtra);
    sfree(string);
