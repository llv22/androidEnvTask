...
{
#ifdef ANDROID
    android_game_state_changed_int("dots", newstate->ndots);

    char *string = snewn(10, char);

    char *gridExtra = snewn((newstate->h + 3) * (newstate->w + 3) * 4, char);
    strcpy(gridExtra, "[");
    int x, y;
    int g = 0;
    char c;
    space *sp;
    for (y = 0; y < newstate->sy; y++) {
        if (y > 0) {
            strcat(gridExtra, ", ");
        }
        strcat(gridExtra, "[");
        for (x = 0; x < newstate->sx; x++) {
            if (x > 0) {
                strcat(gridExtra, ", ");
            }
            sp = &SPACE(newstate, x, y);
            if (sp->flags & F_DOT) {
                c = 'o';
            } else {
                switch (sp->type) {
                    case s_tile:
                        if (sp->flags & F_TILE_ASSOC) {
                            space *dot = sp2dot(newstate, sp->x, sp->y);
                            if (dot && dot->flags & F_DOT) {
                                c = (dot->flags & F_DOT_BLACK) ? 'B' : 'W';
                            } else {
                                c = '?'; /* association with not-a-dot. */
                            }
                        } else {
                            c = ' ';
                        }
