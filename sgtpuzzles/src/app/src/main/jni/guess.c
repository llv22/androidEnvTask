...
#ifdef ANDROID
    int guess;
    if (newstate->solved != 0) {
        guess = newstate->next_go;
    } else {
        guess = newstate->next_go - 1;
    }
    if (guess >= 0) {
        pegrow last_guess =  newstate->guesses[guess];
        char pegExtra[100];
        strcpy(pegExtra, "[");
        char feedbackExtra[100];
        strcpy(feedbackExtra, "[");
        char *string = malloc(sizeof(char)*10);
        for (int p = 0; p < last_guess->npegs; ++p) {
            int peg = last_guess->pegs[p];
            int feedback = last_guess->feedback[p];
            if (p > 0) {
                strcat(pegExtra, ", ");
                strcat(feedbackExtra, ", ");
            }
            sprintf(string, "%d", peg);
            strcat(pegExtra, string);
            sprintf(string,"%d",feedback);
            strcat(feedbackExtra, string);
        }
        strcat(pegExtra, "]");
        strcat(feedbackExtra, "]");
        android_game_state_changed("peg", pegExtra);
        android_game_state_changed("feedback", feedbackExtra);
        sfree(string);
    }
