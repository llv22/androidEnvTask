...
{
#ifdef ANDROID
    android_game_state_changed_int("empty", newstate->gap_pos);
    android_game_state_changed_int("movecount", newstate->movecount);
    ...
    for (y = 0; y < newstate->h; y++) {
        ...
        for (x = 0; x < newstate->w; x++) {
            ...
            int title = newstate->tiles[t];
            sprintf(string, "%d", title);
