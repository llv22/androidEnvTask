...
static jmethodID
	...
	completed,
	onGameStateChanged,
	onGameStateChangedInt,
	onGameStateChangedFloat,
	onSolvedChanged,
	inertiaFollow,
	setKeys;
...
void android_game_state_changed(const char* name, const char* value)
{
    if (!obj) return;
    JNIEnv *env = (JNIEnv*)pthread_getspecific(envKey);
    jstring jsName = (*env)->NewStringUTF(env, name);
    jstring jsValue = (*env)->NewStringUTF(env, value);
    if( jsName == NULL || jsValue == NULL ) return;
    (*env)->CallVoidMethod(env, obj, onGameStateChanged, jsName, jsValue);
}

void android_game_state_changed_int(const char* name, const int value)
{
    if (!obj) return;
    JNIEnv *env = (JNIEnv*)pthread_getspecific(envKey);
    jstring jsName = (*env)->NewStringUTF(env, name);
    if( jsName == NULL) return;
    (*env)->CallVoidMethod(env, obj, onGameStateChangedInt, jsName, value);
}

void android_game_state_changed_float(const char* name, const float value)
{
	if (!obj) return;
	JNIEnv *env = (JNIEnv*)pthread_getspecific(envKey);
	jstring jsName = (*env)->NewStringUTF(env, name);
	if( jsName == NULL) return;
	(*env)->CallVoidMethod(env, obj, onGameStateChangedFloat, jsName, value);
}

void android_solved_changed(int is_solved)
{
	if (!obj) return;
	JNIEnv *env = (JNIEnv*)pthread_getspecific(envKey);
	(*env)->CallVoidMethod(env, obj, onSolvedChanged, is_solved);
}
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *jvm, void *reserved)
{
	...
	onGameStateChanged = (*env)->GetMethodID(env, cls,  "onGameStateChanged", "(Ljava/lang/String;Ljava/lang/String;)V");
	onGameStateChangedInt = (*env)->GetMethodID(env, cls,  "onGameStateChangedInt", "(Ljava/lang/String;I)V");
	onGameStateChangedFloat = (*env)->GetMethodID(env, cls,  "onGameStateChangedFloat", "(Ljava/lang/String;F)V");
	onSolvedChanged = (*env)->GetMethodID(env, cls,  "onSolvedChanged", "(I)V");
