                               ...
                               const game_state *newstate)
{
    ...
    ui->just_made_move = false;
#ifdef ANDROID
	android_game_state_changed_int("gems", newstate->gems);
	android_game_state_changed_int("distancemoved", newstate->distance_moved);
	char *string = snewn(10, char);

	char *gridExtra = snewn(newstate->p.h * newstate->p.w * 4 , char);
	strcpy(gridExtra, "[");
	int i = 0;
	int x;
	int y;
	for (y = 0; y < newstate->p.h; y++) {
		if (y > 0) {
			strcat(gridExtra, ", ");
		}
		strcat(gridExtra, "[");
		for (x = 0; x < newstate->p.w; x++) {
			if (x > 0) {
				strcat(gridExtra, ", ");
			}
			char title = newstate->grid[i];
			sprintf(string, "'%c'", title);
	...
	if (newstate->dead) {
		android_solved_changed(-1);
	} else if (newstate->gems == 0) {
		android_solved_changed(+1);
	} else {
		android_solved_changed(0);
	}
