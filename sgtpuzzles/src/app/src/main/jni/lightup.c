                               ...
                               const game_state *newstate)
{
    ...
    char c;
    char *gridExtra;
    gridExtra = malloc (100 * sizeof *gridExtra);
    strcpy(gridExtra, "[");
    ...
    for (y = 0; y < newstate->h; y++) {
        ...
        for (x = 0; x < newstate->w; x++) {
            c = ' ';
            if (GRID(newstate, flags, x, y) & F_BLACK) {
                if (GRID(newstate, flags, x, y) & F_NUMBERED)
                    c = (char) (GRID(newstate, lights, x, y) + '0');
                else
                    c = '#';
            } else {
                if (GRID(newstate, flags, x, y) & F_LIGHT)
                    c = 'L';
                else if (GRID(newstate, flags, x, y) & F_IMPOSSIBLE)
                    c = 'X';
                else {
                    if (GRID(newstate, lights, x, y) >= 1)
                        c = 'l';
                }
            }
            if (x > 0) {
