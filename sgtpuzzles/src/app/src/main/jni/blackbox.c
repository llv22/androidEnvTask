                               ...
                               const game_state *newstate)
{
    ...
    ui->newmove = false;
#ifdef ANDROID
    android_game_state_changed_int("balls", newstate->nballs);
    android_game_state_changed_int("guesses", newstate->nguesses);
    android_game_state_changed_int("wrong", newstate->justwrong);
    android_game_state_changed_int("lasers", newstate->laserno);
    unsigned int gs_tile;
    int lno;
    int gx, gy, direction;
    int laserval;
    bool tmp;

    char *string = snewn(10, char);

    char *gridExtra = snewn((newstate->h + 2) * (newstate->w + 2) * 4, char);
    strcpy(gridExtra, "[");
    int x;
    int y;
    char p;
    ...
    unsigned int exit;
    unsigned int v;
    for (y = 0; y < newstate->h + 2; y++) {
        if (y > 0) {
            strcat(gridExtra, ", ");
        }
        strcat(gridExtra, "[");
        for (x = 0; x < newstate->w + 2; x++) {
            if (x > 0) {
                strcat(gridExtra, ", ");
            }
            if (y > 0 && y < newstate->h + 1
                && x > 0 && x < newstate->w + 1) { // center / arena
                v = GRID(newstate, x, y);
                p = ' ';
                if (v & BALL_GUESS) {
                    p = 'G';
                } else  if (v & BALL_CORRECT) {
                    p = ' '; // not revealing
                } else  if (v & BALL_LOCK) {
                    p = 'L';
                }
                       ...
                       || (y == 0 && x == newstate->w + 1)
                       || (y == newstate->h + 1 && x == 0)
                       || (y == newstate->h + 1 && x == newstate->w + 1)) { // corners
                p = ' ';
            } else if (y == 0 || y == newstate->h + 1
                       || x == 0 || x == newstate->w + 1) { // laser / range
                tmp = grid2range(newstate, x, y, &lno);
                assert(tmp);
                exit = newstate->exits[lno];
                if (exit == LASER_HIT) {
                    p = 'H';
                } else if (exit == LASER_REFLECT) {
                    p = 'R';
                } else if (exit == LASER_OMITTED) {
                    p = 'O';
                } else if (exit == LASER_WRONG) {
                    p = 'W';
                } else if (exit == LASER_FLASHED) {
                    p = 'F';
                } else if (exit == LASER_EMPTY) {
                    p = '?';
                } else {
                    tmp = range2grid(newstate, lno, &gx, &gy, &direction);
                    assert(tmp);
                    gs_tile = GRID(newstate, gx, gy);
            ...
            sprintf(string, "'%c'", p);
            strcat(gridExtra, string);
        }
        strcat(gridExtra, "]");
    }
    strcat(gridExtra, "]");
    android_game_state_changed("grid", gridExtra);
    sfree(gridExtra);

    sfree(string);

    if (newstate->nwrong == 0
        && newstate->nmissed == 0
        && newstate->nright >= newstate->minballs) {
        android_solved_changed(newstate->nright);
    }
