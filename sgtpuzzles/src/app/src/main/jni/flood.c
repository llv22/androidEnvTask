                               ...
                               const game_state *newstate)
{
    ...
    android_game_state_changed_int("movelimit", newstate->movelimit);
    int w, h;
    char *string = snewn(10, char);
    char *gridExtra;
    gridExtra = snewn(newstate->w * newstate->h * 4 , char);
    strcpy(gridExtra, "[");
    for (h = 0; h < newstate->h; h++) {
        if (h > 0) {
            strcat(gridExtra, ", ");
        }
        strcat(gridExtra, "[");
        for (w = 0; w < newstate->w; w++) {
            if (w > 0) {
                strcat(gridExtra, ", ");
            }
            char colour = newstate->grid[h * newstate->w + w];
            sprintf(string, "%d", colour);
            strcat(gridExtra, string);
        }
        strcat(gridExtra, "]");
    }
    strcat(gridExtra, "]");
    android_game_state_changed("board", gridExtra); // like Flood-It game

    sfree(gridExtra);
    sfree(string);

    android_solved_changed(newstate->complete);
