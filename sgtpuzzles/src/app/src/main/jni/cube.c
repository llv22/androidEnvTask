...
{
#ifdef ANDROID
    android_game_state_changed_int("current", newstate->current);
    android_game_state_changed_int("previous", newstate->previous);
    char *string = snewn(10, char);
    int i;
    long square_face_colour;
    if (newstate->solid->order == 4) { // IF cube
        char *gridExtra;
        gridExtra = snewn(newstate->grid->nsquares * 4 , char);
        strcpy(gridExtra, "[");
        int x,y;
        i = 0;
        for (y = 0; y < newstate->params.d2; y++) {
            if (y > 0) {
                strcat(gridExtra, ", ");
            }
            strcat(gridExtra, "[");
            for (x = 0; x < newstate->params.d1; x++) {
                if (x > 0) {
                    strcat(gridExtra, ", ");
                }
                square_face_colour = GET_SQUARE(newstate, i);
                sprintf(string, "%li", square_face_colour);
    ...
    char *facesExtra;
    facesExtra = snewn(newstate->solid->nfaces * 4 , char);
    int f;
    int face_colour;
    int face_colour_count = 0;
    strcpy(facesExtra, "[");
    for (f = 0; f < newstate->solid->nfaces; f++) {
        if (f > 0) {
            strcat(facesExtra, ", ");
        }
        face_colour = newstate->facecolours[f];
        if (face_colour) {
            face_colour_count++;
        }
        sprintf(string, "%d", face_colour);
        strcat(facesExtra, string);
    }
    strcat(facesExtra, "]");
    android_game_state_changed("faces", facesExtra);
    android_game_state_changed_int("face_count", newstate->solid->nfaces);
    android_game_state_changed_int("face_colour_count", face_colour_count);

    sfree(facesExtra);
    sfree(string);

    android_solved_changed(newstate->completed);
