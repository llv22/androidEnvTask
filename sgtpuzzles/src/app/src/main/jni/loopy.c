...
static void game_changed_state(game_ui *ui, const game_state *oldstate,
    ...
    grid *g = newstate->game_grid;
    grid_face *f;
    f = g->faces; /* 1st face */

    if (newstate->grid_type == 0 && f->order == 4) { // IF cube
        ...
        char *gridExtra = snewn(W * H * 4, char);
        strcpy(gridExtra, "[");
        char c;
        ...
        for (y = 0; y < H; y++) {
            ...
            for (x = 0; x < W; x++) {
                ...
                int ex, ey, ei;
                grid_edge *e;
                for (ei = 0; ei < g->num_edges; ei++) {
                    e = g->edges + ei;
                    x1 = (e->dot1->x - g->lowest_x) / cell_size;
                    x2 = (e->dot2->x - g->lowest_x) / cell_size;
                    y1 = (e->dot1->y - g->lowest_y) / cell_size;
                    y2 = (e->dot2->y - g->lowest_y) / cell_size;
                    ex = x1 + x2;
                    ey = y1 + y2;
                    if (x != ex || y != ey) {
                        continue; // wrong grid edge
                    }
                    if (newstate->line_errors[ei]) {
                        c = (y1 == y2) ? '~' : '/';
                    } else {
                        switch (newstate->lines[ei]) {
                            case LINE_YES:
                                c = (y1 == y2) ? '-' : '|';
                                break;
                            case LINE_NO:
                    ...
                    break; // grid edge found
                }
                int fi, fx, fy;
                for (fi = 0; fi < g->num_faces; fi++) {
                    f = g->faces + fi;
                    ...
                    fy = y1 + y2;
                    if (x != fx || y != fy) {
                        continue; // wrong face edge
                    }
                    c = CLUE2CHAR(newstate->clues[fi]);
                    break; // grid face found
                }
                sprintf(string, "'%c'", c);
    ...
    android_solved_changed(newstate->solved);
