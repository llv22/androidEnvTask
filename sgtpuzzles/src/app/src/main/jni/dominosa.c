                               ...
                               const game_state *newstate)
{
    ...
    char *gridExtra = snewn(newstate->h * newstate->w * 4 , char);
    strcpy(gridExtra, "[");
    i = 0;
    for (y = 0; y < newstate->h; y++) {
        if (y > 0) {
            strcat(gridExtra, ", ");
        }
        strcat(gridExtra, "[");
        for (x = 0; x < newstate->w; x++) {
            if (x > 0) {
                strcat(gridExtra, ", ");
            }
            v = newstate->grid[i];
    ...
    int number;
    char *numbersExtra = snewn(newstate->h * newstate->w * 4 , char);
    strcpy(numbersExtra, "[");
    i = 0;
    for (y = 0; y < newstate->h; y++) {
        if (y > 0) {
        ...
        for (x = 0; x < newstate->w; x++) {
            ...
            number = newstate->numbers->numbers[i];
            sprintf(string, "%d", number);
            strcat(numbersExtra, string);
            i++;
        }
        strcat(numbersExtra, "]");
    }
    strcat(numbersExtra, "]");
    android_game_state_changed("numbers", numbersExtra);
    sfree(numbersExtra);

    int di;
    int n = newstate->params.n;
    unsigned char *used = snewn(TRI(n + 1), unsigned char);
    memset(used, 0, TRI(n + 1));
    for (i = 0; i < newstate->h * newstate->w; i++) {
        if (newstate->grid[i] > i) {
            di = DINDEX(
                    newstate->numbers->numbers[i],
                    newstate->numbers->numbers[newstate->grid[i]]);
            if (used[di] < 2) {
                used[di]++;
            }
        }
    ...
    int clash;
    char *clashExtra = snewn(newstate->h * newstate->w * 4, char);
    strcpy(clashExtra, "[");
    i = 0;
    for (y = 0; y < newstate->h; y++) {
        if (y > 0) {
        ...
        for (x = 0; x < newstate->w; x++) {
            ...
            sprintf(string, "%d", clash);
            strcat(clashExtra, string);
            i++;
        }
        strcat(clashExtra, "]");
    }
    strcat(clashExtra, "]");
    android_game_state_changed("clash", clashExtra);
    sfree(clashExtra);
