                               ...
                               const game_state *newstate)
{
    ...
    char *gridExtra;
    gridExtra = malloc(newstate->w * newstate->h * 4 * sizeof(char));
    strcpy(gridExtra, "[");
    int darkCount = 0;
    int lightCount = 0;
    int r;
    int c;
    int wh = newstate->w * newstate->h;
    int dy;
    int dx;
    int ch = 4;
    int cw = 4;
    int gh = newstate->h * ch + 1;
    int gw = newstate->w * cw + 2;
    char *gridMatrixExtra;
    gridMatrixExtra = malloc(gw * gh * 2 * sizeof(char));
    strcpy(gridMatrixExtra, "[");
    for (r = 0; r < newstate->h; ++r) {
        if (r > 0) {
            strcat(gridExtra, ", ");
            strcat(gridMatrixExtra, ", ");
        }
        strcat(gridExtra, "[");
        strcat(gridMatrixExtra, "[");
        for (c = 0; c < newstate->w; ++c) {
            if (c > 0) {
                strcat(gridExtra, ", ");
                strcat(gridMatrixExtra, ", ");
            }
            bool isDark = (newstate->grid[r * newstate->w + c] & 1) ? true : false;
            if (isDark) {
                darkCount++;
                sprintf(string, "%d", 0);
            } else {
            ...
            for (dy = -1; dy <= 1; ++dy) {
                ...
                for (dx = -1; dx <= 1; ++dx) {
                    ...
                    if ((dy == -1 && r == 0)
                        || (dy == 1 && (r == newstate->h -1))) {
                        strcat(gridMatrixExtra, "-1"); // empty
                    } else if ((dx == -1 && c == 0)
                               || (dx == 1 && (c == newstate->w - 1))) {
                        strcat(gridMatrixExtra, "-1"); // empty
                    } else {
                        bool isNeighbor = newstate->matrix->matrix[
                                (r * newstate->w + c) * wh +
                                ((r + dy) * newstate->w + c + dx)];
                        if (isNeighbor) {
                            strcat(gridMatrixExtra, "1"); // it's neighbours
                        } else {
                            strcat(gridMatrixExtra, "0"); // not in its  neighbours
                        }
                    }
                }
                strcat(gridMatrixExtra, "]");
    ...
    android_game_state_changed("gridMatrix", gridMatrixExtra);
    android_game_state_changed_int("light", lightCount);
    android_game_state_changed_int("dark", darkCount);
    android_game_state_changed_int("moves", newstate->moves);

    sfree(gridMatrixExtra);
    sfree(gridExtra);
    sfree(string);
