...
public class GameView extends View
{
		...
		public void run() {
			...
			if (mScroller.isFinished()) {
				ViewCompat.postOnAnimation(GameView.this, () -> {
					redrawForZoomChange();
					for (EdgeEffectCompat edge : edges) {
						edge.onRelease();
					}
				});
			} else {
	...
	private void zoomMatrixUpdated(final boolean userAction) {
		...
		if (topLeft.x < 0) {
			...
			if (userAction) {
				hitEdge(3, -topLeft.x / wDip, 1 - (lastTouch.y / h));
			}
		...
		if (bottomRight.x > wDip) {
			...
			if (userAction) {
				hitEdge(1, (bottomRight.x - wDip) / wDip, lastTouch.y / h);
			}
		...
		if (topLeft.y < 0) {
			...
			if (userAction) {
				hitEdge(0, -topLeft.y / hDip, lastTouch.x / w);
			}
		...
		if (bottomRight.y > hDip) {
			...
			if (userAction) {
				hitEdge(2, (bottomRight.y - hDip) / hDip, 1 - (lastTouch.x / w));
			}
	...
	public boolean onKeyDown(int keyCode, @NonNull KeyEvent event)
	{
		...
		if (key == 0) {
			...
			if ((exactKey >= 'A' && exactKey <= 'Z') || hardwareKeys.indexOf(exactKey) >= 0) {
				...
				if (key == 0 && (exactKey == 'u' || exactKey == 'r')) {
					key = exactKey;
				}
			}
		}
		if( key == 0 ) {
			return super.onKeyDown(keyCode, event);  // handles Back etc.
		}
	...
	{
		final int color = colour < colours.length ? colours[colour] : Color.MAGENTA;
	...
	{
		if (fillColour != -1) {
			final int color = fillColour < colours.length ? colours[fillColour] : Color.MAGENTA;
			paint.setColor(color);
			paint.setStyle(Paint.Style.FILL);
			canvas.drawPath(p, paint);
		}
		final int color = lineColour < colours.length ? colours[lineColour] : Color.MAGENTA;
		paint.setColor(color);
	...
	void drawText(int x, int y, int flags, int size, int colour, String text)
	{
		...
		if ((flags & ALIGN_V_CENTRE) != 0) {
			y += asc - (asc+desc)/2;
		}
		if ((flags & ALIGN_H_CENTRE) != 0) {
			paint.setTextAlign( Paint.Align.CENTER );
		} else if ((flags & ALIGN_H_RIGHT) != 0) {
