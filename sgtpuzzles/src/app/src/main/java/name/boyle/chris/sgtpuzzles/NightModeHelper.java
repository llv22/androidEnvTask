...
public class NightModeHelper implements SensorEventListener, SharedPreferences.OnSharedPreferenceChangeListener {
	...
	private static final long NIGHT_MODE_AUTO_DELAY = 2100L;
	...
	private NightMode nightMode = RLTask.get().isEnabled() ? NightMode.ON : NightMode.OFF;
	...
	private void applyNightMode(final boolean alreadyStarted) {
		...
		String pref = prefs.getString(NIGHT_MODE_KEY, "auto");
		if (RLTask.get().isEnabled()) {
			pref = "on"; // already ON
		}
		if ("on".equals(pref)) {
